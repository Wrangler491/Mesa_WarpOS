...
void glutRemoveMenuItem(int entry)
{
  ...
  if ((actMenu = glutstuff.curmenu)) {
    ...
    if((gme = stuffGetMenuEntry(entry, actMenu))) {
      stuffLinkOutMenuEntry(gme, actMenu);
      actMenu->needupdate = TRUE;

      FreeVecPooled(glutPool, (ULONG *)gme);
    }
    else
      DEBUGOUT(1, "somethings wrong in glutRemoveMenuItem(%d)\n", entry);
  }
}
