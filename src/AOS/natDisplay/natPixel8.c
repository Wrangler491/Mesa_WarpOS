			  ...
			  CONST GLubyte rgba[][4], const GLubyte mask[])
{
  ...
  DEBUGOUT(1, "nat8WriteRGBAPixelDB(%d)\n", n);

	(GLshort)n--;
  for ((GLshort)n; (GLshort)n >= 0; (GLshort)n--, RGBAi++, Xx++, Yy++)
    if (*mask++)
      *(dbPen(db, *Xx, *Yy)) = PL8_RGBA(amesa, *RGBAb0, *RGBAb1, *RGBAb2);
}
